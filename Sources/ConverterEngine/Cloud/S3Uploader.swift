...
/// Builds SFTP/FTP upload command arguments.
///
/// Uses sftp/scp subprocess calls for file transfer.
///
/// Phase 12.14
public struct SFTPUploader: Sendable {

    /// Build scp arguments for SFTP file upload.
    ///
    /// - Parameters:
    ///   - credential: SFTP credential.
    ///   - localPath: Local file path.
    ///   - remotePath: Remote destination path.
    /// - Returns: Command-line argument array for scp.
    public static func buildSCPArguments(
        ...
        // Port
        if let port = credential.port, port != 22 {
            args += ["-P", "\(port)"]
        }
        ...
        // Destination
        let host = credential.endpoint ?? "localhost"
    ...
    /// Build sftp batch commands for upload.
    ///
    /// - Parameters:
    ///   - localPath: Local file path.
    ///   - remotePath: Remote destination path.
    ///   - createDirectory: Whether to create the remote directory first.
    /// - Returns: SFTP batch command string.
    public static func buildSFTPBatch(
        localPath: String,
        remotePath: String,
        ...
        if createDirectory {
            ...
            commands.append("-mkdir \(remoteDir)")
        }

        commands.append("put \(localPath) \(remotePath)")
        commands.append("quit")

        return commands.joined(separator: "\n")
    }

    /// Validate an SFTP credential.
    ///
    /// - Parameter credential: The credential to validate.
    ...
    public static func validate(credential: CloudCredential) -> [String] {
        ...
        if credential.endpoint == nil || credential.endpoint?.isEmpty == true {
            errors.append("SFTP hostname is required")
        }
        if credential.username == nil || credential.username?.isEmpty == true {
