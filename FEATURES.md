...
| Media Metadata **lookup** / auto-tagging | **Corrected, two ways.** (1) `MetadataLookup`/`MetadataProviders` remain URL builders only, but `TMDBLookupService`/`MusicBrainzLookupService` (also in `Sources/ConverterEngine/Metadata/`) DO perform real HTTP with `URLSession`/`URLRequest`/`JSONDecoder` — the Metadata Tag Editor's "Look Up…" already used them for real before this pass (#493, #502, #503); the "no `URLSession` in that directory" claim this row previously made was already stale. (2) `AutoTagger` no longer has "zero references outside its own file": its lookup-order/confidence-threshold/NFO-path helpers now back a real, opt-in "tag files automatically while converting" switch (Settings › Metadata, off by default) for the app's own encoder, films and music both (#508). TheTVDB/Discogs/FanArt/OpenSubtitles/OMDb are still URL builders with no key UI; `AutoTagger`'s rename/artwork helpers, TV episodes, and a CLI flag remain unused, unbuilt follow-ups; the `meedya-convert` CLI, the API server's standalone engine, and any encoding pipeline never auto-tag | #205, #493, #508 |
