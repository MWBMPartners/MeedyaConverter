...
| `gitleaks` (CLI route) | binary tarball, version-pinned | github.com/gitleaks/gitleaks releases | 2026-06-30 | Per user's global `.claude/CLAUDE.md` guidance — CLI route, MIT-licensed binary, no action wrapper (the wrapper requires a paid org licence). |

**Bundled binaries**: `ffmpeg` + `ffprobe` are pulled from official Homebrew in `release.yml` / `testflight.yml`. brew formula is the canonical source — no third-party tap shadowing. The bundled binaries are then code-signed with the user's Developer ID Application certificate before notarisation, so a compromised brew build would surface as a signature mismatch on the next clean install.
