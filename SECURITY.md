...
| F-011 | Medium | T7 — `scripts/bundle-ffmpeg.sh` downloads `ffmpeg`/`ffprobe`/`ffplay` from `www.osxexperts.net` over a static per-version `.zip` URL with **no SHA-256 or signature verification**, and `release.yml` then code-signs + notarises the result under MWBM's Developer ID. A compromised or silently re-published upstream artefact would be shipped to every user Gatekeeper-approved. | **machinery fixed** (fail-closed); checksum population pending (maintainer, trusted network) | 26 (review) + post-review | Found by the independent adversarial review after VERIFY (workflow wf_049f82a0). `bundle-ffmpeg.sh` is pre-existing (not introduced by the autopilot run), but the autopilot's `release.yml` FFmpeg-bundling step is what wraps the unverified download in a trusted signature, so the risk became load-bearing on this branch. HTTPS guarantees transport integrity only; the URLs are not content-addressed. **Fix landed (post-review commit):** `bundle-ffmpeg.sh` now computes the SHA-256 of each downloaded `.zip` **immediately after curl, before unzip**, and compares it to a pinned value in the new `scripts/ffmpeg-checksums.txt` (keyed by version + arch + tool). It **FAILS CLOSED** — exit 6 if no pin exists for the (version, arch, tool), exit 7 on mismatch — so an unverified or tampered binary can never reach the sign/notarise step. A dedicated `--refresh-checksums <version> <arch>` mode records the pins (trust-on-first-use); it is a deliberate, separate command that NEVER runs during a normal build (auto-refresh would defeat the purpose). Because the pin is per-version, a version bump is a two-part edit: change `VERSION` + re-run `--refresh-checksums` + commit the manifest together (documented in the script header). **Remaining maintainer step:** `scripts/ffmpeg-checksums.txt` ships with NO entries — it must be populated once from a trusted network (`scripts/bundle-ffmpeg.sh --refresh-checksums 7.1.2 arm64` and `... x86_64`), ideally cross-checked against an independent build. Until then the release build fails closed, by design. NOTE: the script's osxexperts.net download URLs did not resolve from the review sandbox (404/503) — the naming scheme may be stale; confirm it when populating. The dependency-manifest note above was also corrected (it had wrongly claimed Homebrew + signature-mismatch protection). |
