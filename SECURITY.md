...
| F-002 | Medium | T2 — `ScriptingBridge.swift:127` AppleScript bridge accepted arbitrary `output` string as URL with no validation | fully closed — defence-in-depth complete | 14, 25 | RED audit found ~20 sites where `URL.appendingPathComponent(component)` is called with a `component` derived (directly or via `URL.lastPathComponent`) from user input. **Most are not directly exploitable**: `URL.lastPathComponent` doesn't expose path separators, and Foundation's filesystem APIs cannot create a file literally named `..`. **The narrow real vector was `ScriptingBridge.swift:127`** — `URL(fileURLWithPath: output)` where `output` came directly from an AppleScript caller as a raw string with no normalisation. A malicious script could pass `"../../../etc/passwd"` and (subject to the user's filesystem permissions) overwrite arbitrary files outside the user's intended output area. **Fix landed in commit on autopilot/2026-06-30**: new `Sources/ConverterEngine/Utilities/PathSanitizer.swift` with two complementary helpers — `PathSanitizer.sanitizeFilenameComponent(_:)` (silently strips `/`, `\`, `..`, NUL, leading whitespace, trailing whitespace+dots from filename components, with `unnamed` placeholder for empty/dot-only results), and `URL.isContained(within:)` extension (string-only `.standardized`-based descendant check, with documented decision to NOT resolve symlinks because macOS's `/private/tmp` ↔ `/tmp` redirection produces inconsistent results between existing and non-existing paths). The ScriptingBridge.encode method now rejects any output URL not contained within the user's home directory, returning a clear `ERROR:` string the AppleScript caller can surface. **Regression coverage**: new `Tests/ConverterEngineTests/PathSanitizerTests.swift` with 15 tests covering empty/dot/separator/traversal inputs, idempotence, the load-bearing "sanitised input cannot escape parent" assertion, and the containment check including the prefix-collision case (`/private/tmp` ≠ `/private/tmp-attack/`). All pass. **POLISH-tier follow-up**: ~17 other `appendingPathComponent` call sites (`MultiOutputView`, `BackgroundRemovalView`, `Intents/ConvertMediaIntent`, etc.) could be defensively migrated to the sanitiser even though they aren't directly exploitable, just to close the door on a future regression where `URL.lastPathComponent` semantics change or someone passes a raw string. Queued for a POLISH cycle, not blocking v0.1.0. **POLISH Cycle 25 fix (first round, 10 sites)**: `PathSanitizer.sanitizeFilenameComponent(...)` wrapped around `SlateGeneratorView`, `MultiOutputView`, `ResumableJobsView`, `ScheduleView`, `BackgroundRemovalView`, `PluginManagerView`, `FFmpegPreviewView`, `ImageConversionView`, `BatchRenameView`, and `Intents/ConvertMediaIntent` — all deriving their component from `URL.lastPathComponent` of a user-chosen file, or a user-typed profile/label string. **Follow-up defence-in-depth sweep (2026-07-18, fully closing the audit tail)**: a fresh `grep -rn appendingPathComponent Sources/` plus manual read of every hit found 13 further sites that were user-derived but not covered by Cycle 25 — mostly ConverterEngine-layer twins of already-fixed View-layer call sites, plus two genuinely new surfaces. Migrated: `BatchRenamer.apply` (`Utilities/BatchRenamer.swift`) — the highest-value fix, since a rename rule's `replaceWith` text is fully user-controlled and previously flowed unsanitised into the destination path (unlike the `lastPathComponent`-only sites, this one is not just "future-proofing"); `EncodeMediaAction.outputURL` (`Automator/EncodeMediaAction.swift`) — the Automator-action sibling of the already-fixed `ConvertMediaIntent` Shortcuts surface; `FilenameTemplate.resolve` (both overloads, `Utilities/FilenameTemplate.swift`) — the core user-editable filename-template engine, whose own ad hoc `sanitiseFilename` strips `/:\` but not a bare `..`, which matters because untrusted media metadata (F-006) can flow into `{title}`; `OutputPathResolver.resolveOutputPath` (`Utilities/OutputPathResolver.swift`) — the app's canonical output-path resolver; `ProfileSharing.exportAsURL` and `ComparisonCapture.captureFromMultipleProfiles` (both keyed on user-typed `profile.name`); `PreviewGenerator.previewOutputPath`, `BackgroundRemover.batchRemoveBackgrounds`, `EncodingPipeline.buildStepArguments` (5 call sites), `VideoTrimmer` (3 call sites), and `WatchFolderManager.outputPath` — all the same `baseName`/`stem` + literal-suffix pattern already fixed elsewhere; and the CLI target's `BatchCommand`/`EncodeCommand` (`Sources/meedya-convert/Commands/`) for GUI/CLI parity. **Deliberately left unchanged**: `WatchFolderManager`'s and `OutputPathResolver`'s `relativePath` variables — these are multi-segment relative *directory* paths that intentionally contain `/` to preserve nested subdirectory structure in mirror/recursive modes, so running them through `sanitizeFilenameComponent` (which flattens every `/` to `_`) would be a functional regression, not a security fix; `ManifestGenerator`/`ManifestCommand`'s `variant.label` (bounded to a fixed set of built-in preset strings, e.g. `"1080p"`, never user-editable); `SubtitleTonemapPipeline`'s `ext` (bounded to `"sup"`/`"ass"` via a fixed codec-name switch); `BlurayReader`/`DVDReader`/`FFmpegBundleManager` directory-structure literals and printf-formatted numeric filenames; and every UUID- or compile-time-literal-keyed path elsewhere in `Sources/`. New regression coverage: `Tests/ConverterEngineTests/BatchRenamerF002Tests.swift` (2 tests) exercises `BatchRenamer.apply` end-to-end against real files in a temp directory, asserting a path-traversal `replaceWith` rule cannot move a file outside its own directory, and that ordinary renames are byte-for-byte unaffected. Build clean; full suite 1077/1077 pass (1075 pre-existing + 2 new). F-002 is now considered **fully closed** — no known `appendingPathComponent` call site in `Sources/` receives an unsanitised user-derived filename component. |
...
| F-009 | Low | T7 — `Package.swift` line 76 declares `.package(url: "https://github.com/MWBMPartners/MeedyaSuite-core.git", branch: "main")` (a branch pin, not a tag pin), so any commit pushed to that sibling repo's main branch lands in the next ConverterEngine build the moment SPM resolves dependencies. A branch pin is the canonical supply-chain weakness: even a sibling repo controlled by the same publisher accumulates risk if a contributor account is compromised or a malicious PR slips through review | accepted-risk | 21 | RED audit of supply-chain surface. The `MeedyaSuite-core` dependency is **gated behind the `SUITE_CORE=1` env var** (build-time opt-in, not the default) AND has a `SUITE_CORE_PATH` local-checkout fallback for development. For v0.1.0 the env var is not set by any release.yml/testflight.yml/build.yml path, so the branch pin does not affect shipped artefacts — it's only reachable from a local opt-in build by a developer who has decided to integrate the Suite-Core split. **Risk accepted** with explicit documentation: this is a MWBM Partners-controlled sibling repo, the same publisher is on both sides of the dependency, and the env-var gate keeps the loose pin off the v0.1.0 ship path. **Migration**: when MeedyaSuite-core publishes a semver-tagged release (`v0.1.0` etc.), Package.swift should switch to `from: "0.1.0"` (`.upToNextMajor`) to match the convention every other SPM dep in this manifest uses. Surfaced in the gate-ledger as G-014. |
| F-010 | Low | T7 — `.github/workflows/*.yml` pin GitHub Actions using major-version tags (`@v4`) rather than SHA pins (`@a8b9c0d…40-char`). The maintainer of an action — or a compromised maintainer account — can re-tag `v4` to a malicious commit and every workflow run will silently pick it up on the next dispatch. The full attack chain requires either a maintainer compromise or a deliberate malicious release, but the cost of defence (one-time SHA-pinning sweep + a Dependabot config to track upstream releases) is low | **mostly-fixed** (Cycle 23) — SHA-pin sweep still open | 21, 23 | 19 `uses:` references audited across 8 workflow files (`build.yml`, `beta-alpha.yml`, `codeql.yml`, `release.yml`, `dev-build.yml`, `testflight.yml`, `dependency-review.yml`, plus the security-check workflow added this cycle). All actions are GitHub-published (`actions/checkout`, `actions/cache`, `actions/upload-artifact`, `actions/dependency-review-action`, `github/codeql-action/*`) — no third-party actions. **All use major-version tag pins**: `@v4` for everything. This is the GitHub-recommended baseline and matches the v0.1.0 security floor; tag pins are NOT loose-branch-pinned (a `@v4` is at least an immutable release artefact in the moment, even if it CAN be re-tagged), so the immediate risk is bounded by GitHub's account-takeover defences for blue-tick action publishers. **Cycle 21 fix**: new `scripts/security-check-action-pins.sh` is a static linter that fails CI if ANY new `uses: actor/repo@<branch>` reference appears (branch pins are stricter than the current `@v4` baseline). The script's allowlist accepts `@vX`, `@vX.Y`, `@vX.Y.Z`, and full 40-character SHAs; everything else is a violation. Smoke-tested positive (clean repo passes) and negative (synthesised `@main` and short non-SHA both flag). **Cycle 23 fix**: gate framework now wired into CI. New `.github/dependabot.yml` configures Dependabot for both `github-actions` and `swift` ecosystems on a weekly cadence, target-branch=`autopilot/2026-06-30`, so every action / SPM bump arrives as a reviewable PR with diff rather than a silent re-tag. New `.github/workflows/security-check.yml` runs the linter on every push to main/run-branch and every PR that touches `.github/workflows/**` or `scripts/security-check-*.sh`. The result: **branch pins are now CI-blocked**, dependency upgrades flow via PR review, and the one-shot SHA-pin sweep becomes truly optional. **Residual work** (kept in gate-ledger G-015, now de-prioritised): the one-shot conversion of every existing `@v4` to its current 40-char SHA. With the CI gate in place this is hygiene rather than a security necessity — and is best done as a focused PR with each SHA verified against the current published action binary. **Post-VERIFY review fix (2026-07-01):** `.github/dependabot.yml` had `target-branch: autopilot/2026-06-30` (a transient branch absent from origin) which would have made Dependabot open zero PRs on merge to main — the key is now removed so it tracks the default branch. The pin-linter was also widened to catch mutable `docker://image:tag` references and to scan `.yaml` (not just `.yml`) workflow files. |
| F-011 | Medium | T7 — `scripts/bundle-ffmpeg.sh` downloads `ffmpeg`/`ffprobe`/`ffplay` from `www.osxexperts.net` over a static per-version `.zip` URL with **no SHA-256 or signature verification**, and `release.yml` then code-signs + notarises the result under MWBM's Developer ID. A compromised or silently re-published upstream artefact would be shipped to every user Gatekeeper-approved. | **fixed** (first-party universal, SHA256SUMS-verified, fail-closed) | 26 (review) + post-review | Found by the independent adversarial review after VERIFY (workflow wf_049f82a0). `bundle-ffmpeg.sh` is pre-existing, but the autopilot's `release.yml` FFmpeg-bundling step wraps the download in a trusted signature, so the risk became load-bearing on this branch. HTTPS guarantees transport integrity only; the URLs are not content-addressed. **Fix (final state):** (1) **Sole source is the first-party mirror `MeedyaSuite/MeedyaDL-Tools`**, pinned to an immutable dated release tag (`MDLT_TAG`, currently `2026-07-05.3`). There is **no third-party fallback in this repo** — the mirror is the only source, so the supply chain is entirely first-party. The mirror itself fetches **real native** static builds for both arches — macOS arm64 from osxexperts.net (browser-UA + retry, version-token-derived URL) and macOS x86_64 from evermeet.cx — repackages them as `<tool>-macos-<arch>.tar.gz`, and publishes a **`SHA256SUMS`** asset alongside them (MeedyaDL-Tools PR #20, closing MeedyaDL-Tools#15/#19). (2) **`bundle-ffmpeg.sh` builds a UNIVERSAL (arm64 + x86_64) binary per tool** by fetching both per-arch archives and `lipo -create`-combining them, then asserts the result reports both arches (`lipo -archs`, else exit 8). Universal was chosen deliberately: Apple has signalled Rosetta 2 will be decommissioned, so shipping a real arm64 slice from day one is required. (3) **Fail-closed SHA-256 verification against the pinned release's own `SHA256SUMS`, before unpack**: missing `SHA256SUMS` or a missing pin → exit 6, hash **mismatch** → exit 7 (always fatal, even for the optional `ffplay`). Nothing unverified reaches the sign/notarise step. The trust root is the first-party tagged release's own `SHA256SUMS`; **no local hashes are maintained** — the previous URL-keyed `scripts/ffmpeg-checksums.txt` bridge (and its `--refresh-checksums` mode) has been **removed** now that the mirror publishes checksums. (4) `ffmpeg` + `ffprobe` are **required** (either arch missing → fatal); `ffplay` (in-app preview only) is best-effort but still hash-gated. **Verified locally:** all three tools build universal (`x86_64 arm64`), SHA-256 checks pass against `2026-07-05.3`'s `SHA256SUMS`, and the staged `ffmpeg`/`ffprobe` run (`ffmpeg version 8.1`). `release.yml` builds the app itself universal (`swift build -c release --arch arm64 --arch x86_64`, bin path resolved via `--show-bin-path`). MeedyaDL-Tools also tracks the arm64 ffmpeg version on its regular update-check schedule so newer builds flow through automatically; bumping `MDLT_TAG` is the single deliberate edit to pick one up. |
| F-012 | Low (nit) | T5 — `FFmpegProbe`'s watchdog escalation `kill(process.processIdentifier, SIGKILL)` (and the SIGTERM timer) had a PID-reuse TOCTOU: between the handler's `isRunning` check and the signal, the child could exit, be reaped by `waitUntilExit()` (freeing its PID), and have the PID reused by another of the user's processes — which would then receive the signal. | mitigated | 26 (review) + post-review | Found by the independent review (corrected to nit). Realistic trigger is razor-thin: a pathological ffprobe that exits on its own microseconds before the escalation deadline (~63s). **Mitigation landed:** the finished-check, the `isRunning` re-check, and the actual `terminate()`/`kill()` are now folded into a SINGLE `ProbeRunState` lock acquisition (`terminateIfActive`/`killIfActive`), and `markFinished()` is set immediately after `waitUntilExit()` returns — BEFORE the timers are cancelled. Since `DispatchSource.cancel()` does not interrupt an already-executing handler, a handler that begins after the run completes now observes `_finished` under the lock and does nothing. This removes the realistic "timer fires after normal completion" window. A sub-microsecond kernel-level race (child reaped between the in-lock `isRunning` check and the `kill(2)` syscall) remains inherent to signalling a `Foundation.Process` by PID and is accepted — documented inline on `ProbeRunState.killIfActive`. |

## Dependency manifest (T7)

Snapshot at Cycle 21 (commit autopilot/2026-06-30 head). Updated whenever a dependency is added, removed, or version-bumped.

| Dependency | Version pin | Source | Audited at | Notes |
|------------|-------------|--------|------------|-------|
| `Sparkle` | `from: "2.6.0"` (`.upToNextMajor` → < 3.0.0) | github.com/sparkle-project/Sparkle | 2026-06-30 | DIRECT-build only. Apple-blessed framework. Active maintenance. v2 EdDSA signature chain is the protection F-003 documents the activation gates for. |
| `ffmpeg-kit` | `from: "6.0.0"` (`.upToNextMajor` → < 7.0.0) | github.com/arthenica/ffmpeg-kit | 2026-06-30 | APP_STORE-build only. arthenica is the canonical maintainer. The library wraps ffmpeg + codec libraries into an XCFramework suitable for App Store sandboxed distribution. |
| `swift-argument-parser` | `from: "1.5.0"` (`.upToNextMajor` → < 2.0.0) | github.com/apple/swift-argument-parser | 2026-06-30 | Apple-published. Used by the CLI target. |
| `MeedyaSuite-core` | `branch: "main"` | github.com/MWBMPartners/MeedyaSuite-core | 2026-06-30 | **Branch pin — see F-009.** Gated behind SUITE_CORE=1 env var; not on v0.1.0 ship path. Migrate to `from: "X.Y.Z"` once the sibling repo cuts a tagged release. |
| `actions/checkout` | `@v4` | github.com/actions/checkout | 2026-06-30 | GitHub-published. Tag pin per F-010; queued for SHA conversion in POLISH. |
| `actions/cache` | `@v4` | github.com/actions/cache | 2026-06-30 | Same as above. |
| `actions/upload-artifact` | `@v4` | github.com/actions/upload-artifact | 2026-06-30 | Same as above. |
| `actions/dependency-review-action` | `@v4` | github.com/actions/dependency-review-action | 2026-06-30 | Same as above. Already provides a CI gate for new SPM dep risk. |
| `github/codeql-action/init` | `@v4` | github.com/github/codeql-action | 2026-06-30 | Same as above. |
| `github/codeql-action/analyze` | `@v4` | github.com/github/codeql-action | 2026-06-30 | Same as above. |
| `gitleaks` (CLI route) | **NOT WIRED (planned)** | github.com/gitleaks/gitleaks releases | 2026-06-30 | Listed for planning only — there is currently **no gitleaks step** in any workflow (see the scanner-coverage ledger above; it's a Cycle 13+ candidate). When wired it will use the MIT-licensed CLI binary (no action wrapper, which requires a paid org licence) pinned to a specific release + SHA-256. Kept in this table as a visible reminder, explicitly marked not-active so the manifest doesn't overstate coverage. |

**Bundled binaries**: `ffmpeg` / `ffprobe` / `ffplay` are staged by `scripts/bundle-ffmpeg.sh` from the first-party mirror **`MeedyaSuite/MeedyaDL-Tools`** (pinned to an immutable dated release, `MDLT_TAG`), then code-signed with the user's Developer ID Application certificate and notarised as part of the Direct DMG. Each tool is assembled **universal (arm64 + x86_64)** via `lipo` from the mirror's two per-arch archives (real native builds: arm64 from osxexperts.net, x86_64 from evermeet.cx — both fetched by the mirror, not this repo). ✅ **Integrity (F-011, fixed):** every archive is **SHA-256-verified against the pinned release's own `SHA256SUMS` before unpack, fail-closed** (missing checksums/pin → exit 6, mismatch → exit 7), so nothing unverified reaches the sign/notarise step; the trust root is the first-party tagged release, and **no local hashes are maintained** (the former `scripts/ffmpeg-checksums.txt` was removed once the mirror began publishing `SHA256SUMS`). (An earlier revision of this note claimed the binaries came from "official Homebrew" and that tampering "would surface as a signature mismatch" — both were incorrect; corrected here.) See F-011 above for the full detail.
