...
| F-011 | Medium | T7 — `scripts/bundle-ffmpeg.sh` downloads `ffmpeg`/`ffprobe`/`ffplay` from `www.osxexperts.net` over a static per-version `.zip` URL with **no SHA-256 or signature verification**, and `release.yml` then code-signs + notarises the result under MWBM's Developer ID. A compromised or silently re-published upstream artefact would be shipped to every user Gatekeeper-approved. | **fixed for the first-party path** (MeedyaDL-Tools ffmpeg, verified); ffprobe/ffplay/Intel pending mirror coverage | 26 (review) + post-review | Found by the independent adversarial review after VERIFY (workflow wf_049f82a0). `bundle-ffmpeg.sh` is pre-existing, but the autopilot's `release.yml` FFmpeg-bundling step wraps the unverified download in a trusted signature, so the risk became load-bearing on this branch. HTTPS guarantees transport integrity only; the URLs are not content-addressed. **Fix landed (post-review commits):** (1) **Source policy switched to MeedyaDL-Tools-first.** `bundle-ffmpeg.sh` now obtains tools from the first-party mirror **MeedyaSuite/MeedyaDL-Tools**, pinned to an immutable dated release tag (`MDLT_TAG`), and only falls back to a third-party static build (osxexperts.net) for (tool, arch) the mirror doesn't yet publish. (2) **Every archive is SHA-256-verified before unpack, fail-closed** — exit 6 when a download has no pin / no source, exit 7 on mismatch — so nothing unverified reaches the sign/notarise step. (3) **Verification precedence:** first the mirror's own `SHA256SUMS` fetched from the same pinned release (target end-state, filed as MeedyaDL-Tools#19); else a locally-committed pin in the new `scripts/ffmpeg-checksums.txt` (URL-keyed), used for fallback artefacts and as a bridge until #19 lands. A deliberate `--refresh-checksums` mode records local pins; it never runs during a normal build. **Status now:** `ffmpeg` (macOS arm64) is sourced first-party from MeedyaDL-Tools `2026-07-02` and **pinned + verified** (sha `6137374f…cb746b`, confirmed via the script's own curl path) — the happy path builds and the staged binary runs. `ffprobe`/`ffplay` and macOS **x86_64 (Intel)** ffmpeg are **not yet available** from the mirror (tracked upstream: **MeedyaDL-Tools#15** for ffprobe/ffplay, plus an Intel-coverage note on that issue) and the osxexperts fallback URLs did not resolve from the review sandbox (404/503, likely a stale naming scheme), so those tools currently **fail closed with a clear actionable error**. Full green requires MeedyaDL-Tools#15 (+ #19), after which the fallback can be dropped entirely. The dependency-manifest note above was also corrected (it had wrongly claimed Homebrew + signature-mismatch protection). |
...
**Bundled binaries**: `ffmpeg` / `ffprobe` / `ffplay` are downloaded by `scripts/bundle-ffmpeg.sh` from **`www.osxexperts.net`** (a static per-version `.zip` URL), then code-signed with the user's Developer ID Application certificate and notarised as part of the Direct DMG. ⚠️ **Integrity gap (F-011, open):** the download is **not** checksum- or signature-verified before signing — the binary is re-signed under MWBM's Developer ID regardless of content, so a compromised/re-published upstream artefact would be shipped Gatekeeper-approved. HTTPS protects transport only, not source integrity, and the URLs are not content-addressed. (An earlier revision of this note claimed the binaries came from "official Homebrew" and that tampering "would surface as a signature mismatch" — both were incorrect: the source is osxexperts.net and re-signing masks any mismatch. Corrected here.) See F-011 below for the fix plan.
