...
| F-011 | Medium | T7 — `scripts/bundle-ffmpeg.sh` downloads `ffmpeg`/`ffprobe`/`ffplay` from `www.osxexperts.net` over a static per-version `.zip` URL with **no SHA-256 or signature verification**, and `release.yml` then code-signs + notarises the result under MWBM's Developer ID. A compromised or silently re-published upstream artefact would be shipped to every user Gatekeeper-approved. | open — surfaced by the post-VERIFY independent review; fix proposed | 26 (review) | Found by the independent adversarial review after VERIFY (workflow wf_049f82a0). `bundle-ffmpeg.sh` is pre-existing (not introduced by the autopilot run), but the autopilot's `release.yml` FFmpeg-bundling step is what wraps the unverified download in a trusted signature, so the risk became load-bearing on this branch. HTTPS guarantees transport integrity only; the URLs are not content-addressed. **Fix plan (deferred to a focused PR — needs the real published SHA-256 of each pinned build):** (1) record the expected SHA-256 of each `ffmpeg{VER}{arch}.zip` / `ffprobe` / `ffplay` artefact in `bundle-ffmpeg.sh` (or a sidecar `ffmpeg-checksums.txt`); (2) `shasum -a 256 -c` the download BEFORE unzip/sign, failing the build on mismatch; (3) optionally migrate the source to a content-addressed / official-Homebrew-bottle URL. Until then this is an accepted, documented risk gated by the fact that a release is cut manually by the maintainer, not on every push. The dependency-manifest note above was also corrected — it previously (incorrectly) claimed the binaries came from Homebrew and that tampering would surface as a signature mismatch. |
